...
# ── Krok 2b: Zapisz token do SSM jako SecureString ────────────
step "Krok 3: Bezpieczny zapis tokenu do SSM"

log "Zapisuję token HuggingFace do SSM Parameter Store (SecureString, szyfrowany KMS)..."
aws ssm put-parameter \
  ...
  --region "$REGION" \
  > /dev/null
ok "Token zapisany zaszyfrowany w SSM: /bielik-bedrock/hf-token"

unset HF_TOKEN  # usuń token z pamięci skryptu

# ── Krok 3: Zapisz szablon CFN ────────────────────────────────
#
#  Template jest wbudowany bezpośrednio w ten skrypt (heredoc).
#  Nie wymaga dostępu do GitHub ani żadnego zewnętrznego URL.
#
step "Krok 4: Przygotowanie szablonu CloudFormation"

cat > "$TEMPLATE_FILE" << 'CFNEOF'
AWSTemplateFormatVersion: '2010-09-09'
Description: >
  poznajawspl — Bielik 11B na Amazon Bedrock.
...
Resources:
  ...
  ModelBucket:
    ...
    Properties:
      ...
      Tags:
        - Key: Project
          Value: poznajawspl-Bielik

  BedrockImportRole:
    Type: AWS::IAM::Role
    Properties:
      RoleName: !Sub "poznajawspl-Bielik-Bedrock-${AWS::Region}"
  ...
  BielikImportProject:
    ...
    Properties:
      Name: poznajawspl-Bielik-Import
          ...
          phases:
            ...
            pre_build:
              commands:
                ...
                - export HF_TOKEN=$(aws ssm get-parameter --name "$HF_TOKEN_SSM_PATH" --with-decryption --query Parameter.Value --output text)
      ...
      LogsConfig:
        CloudWatchLogs:
          Status: ENABLED
          GroupName: /aws/codebuild/poznajawspl-Bielik-Import
      Tags:
        - Key: Project
          Value: poznajawspl-Bielik
...
Outputs:
  ...
  WatchLogsCommand:
    ...
    Value: !Sub "aws logs tail /aws/codebuild/poznajawspl-Bielik-Import --follow --region ${AWS::Region}"
  ModelBucketName:
...
step "Krok 5: Deploy stacka CloudFormation"
...
aws cloudformation deploy \
  ...
  --tags \
    Project=poznajawspl-Bielik \
  --no-fail-on-empty-changeset

ok "Stack CFN gotowy ✅"

# ── Krok 4: Odczyt Outputs ────────────────────────────────────
step "Krok 6: Odczyt konfiguracji"
...
step "Krok 7: Uruchamianie importu (CodeBuild)"
